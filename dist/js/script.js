...
var arr = [];
var userArrLength = +prompt('Put LENGTH of your arrow', '5');
for (var i = 0; i <= userArrLength - 1; i++) {
  var item = +prompt("Put your numbers into arrow which has length only ".concat(userArrLength, " items"), '');
  if (!isNaN(item)) {
    arr.push(item);
  } else {
    arr.push('NaN');
    console.log("\xEDt is not a number To arrow will be put \"NaN\" ");
  }
}
